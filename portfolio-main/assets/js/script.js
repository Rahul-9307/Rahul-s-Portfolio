...
class PortfolioApp {
    ...
    generateProjectButtons(project) {
        ...
        if (project.liveUrl && project.liveUrl !== '#' && project.liveUrl !== null) {
            ...
            const liveIcon = project.liveIcon || 'fas fa-eye';
            buttons.push(`
    ...
    //             this.showNotification('❌ Failed to send message. Please try emailing me directly at rahulgpatil165@gmail.com', 'error');
    //         })
    //         .finally(() => {
...
initializeEmailJS() {
    emailjs.init("_1fB-2li5YeeVLaVy"); 
}




// ---------------- CONTACT FORM HANDLER (FINAL VERSION) ----------------
...
handleContactForm(e) {
   ...
   const formData = {
    ...
    emailjs.send(
        "service_pib2szt",    // ← Your Service ID
        "template_mfop1dl",   // ← Your Template ID
        formData
    )
    .then(() => {
    ...
    .catch((error) => {
        ...
        this.showNotification(
            "❌ Failed to send message. Please try emailing me directly at rahulgpatil165@gmail.com",
            "error"
        );
    })
    .finally(() => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const hiddenCerts = document.querySelectorAll(".hidden-certificate");
